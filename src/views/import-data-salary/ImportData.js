...
import LovSelectCompany from './LovSelectCompany';

const callImportData = async (data = {}, dateRage, companyId) => {
...
const ImportData = () => {
  ...
  const { mutate: saveFile, isLoading: isSaving } = useMutation((currentData) => callImportData(currentData, dateRage, companyId), {
    onSuccess(res) {
      toast('success');
      setShow(true);
  ...
  return (
    <div>
      ...
      <PageTitle
        ...
        isDisabled={companyId !== undefined}
      />
      <Row>
        ...
        <Col md="4" className="mb-2">
          ...
          <LovSelectCompany isClearable onChange={(e) => setCompanyId(e)} />
        </Col>
      </Row>
      <Card
